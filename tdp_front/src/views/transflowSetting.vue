<template>
    <div id="task_new">
        <el-button color="#705DEB" id="new_task11" @click="update_task">修改任务设置</el-button>
        <el-button color="#0a8895" id="new_task12" @click="back">返回</el-button>
        <div id="taskx1">
        ...
        <div id="taskx2">
            ...
            <div v-if="selectedPanel === 'zaixian'">
                <el-button color="#705DEB" id="add_url" @click="add_url()"
                    v-if="selectedPanel == 'zaixian'">添加数据源</el-button>
...
//查询任务详情,并将详情更新到全局变量
async function get_task_info(ip,href) {
    try {
        console.log(href)
        const url = new URL(href);
        console.log(url)
        const trip_id = url.searchParams.get("trip_id")
        const response = await fetch(`http://${ip}:31000/api/v1/trip/${trip_id}`);
        // 检查响应是否成功  
        if (!response.ok) {
            throw new Error(`HTTP error! Status: ${response.status}`);
        }
        ...
        list.trip_static_source_list.push(x)
        task_info.value = list
